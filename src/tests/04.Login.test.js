...
describe('Testa a página de Login da aplicação', () => {
  ...
  it('Teste 11 - Testa se o usuário é redirecionado para a tela de Ranking ao clicar no botão "Ranking"', () => {
    const { history } = renderWithRouterAndRedux(<App />);
  ...
  it('Teste 12 - Testa se o usuário é redirecionado para a tela de About ao clicar no botão "About & Rules"', () => {
    const { history } = renderWithRouterAndRedux(<App />);

    const aboutButton = screen.getByRole('button', { name: 'About & Rules' });
